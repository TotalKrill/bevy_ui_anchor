fn main() {
    println!(
        "The Rapier example is disabled until bevy_rapier3d and bevy_editor_cam have Bevy 0.19-compatible crates.io releases."
    );
}

// Original Rapier example, kept here so it can be reinstated once the
// dependencies above can be restored in Cargo.toml.
#[cfg(any())]
mod disabled_rapier_example {
    use bevy::{
    ...
    pub fn setup_physics(
        ...
        commands.spawn((
            ...
            MeshMaterial3d(
                materials.add(StandardMaterial::from_color(color::palettes::basic::SILVER)),
            ),
        ));
